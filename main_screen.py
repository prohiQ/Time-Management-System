...
def check_internet():
    ...
    root.after(10000, check_internet)
...
def insert_data_to_today_tasks():
    ...
    try:
        ...
        if len(result_tasks) != 0:
            ...
            number_of_tasks_label.place(x = 140, y = 6)
            corrected_result = f"  No new tasks. Enjoy your day!"
            today_tasks_list.insert("end", corrected_result)
        ...
        number_of_tasks_label.place(x = 140, y = 6)
        corrected_result = f"  No tasks for today"
        today_tasks_list.insert("end", corrected_result)
    progress_bar_of_day()
...
def task_done():
    ...
    try:
        if row is None:
            messagebox.showerror("ERROR", "OOPS! No matching task found in database.")
            return
        else:
            row_id = row[0]
            tasks_cursor.execute("SELECT * FROM tasks WHERE id=?", (row_id,))
            row_to_copy = tasks_cursor.fetchone()
            done_cursor.execute("INSERT INTO tasks_done VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?)", row_to_copy[1:])
            done_db.commit()
            ...
            progress_bar_of_day()
            messagebox.showinfo("Congratulations", f"Task {corrected_done} has been DONE!")
...
def see_task():
    ...
    def insert_values():
        ...
        task_to_be_viewed = today_tasks_list.get((today_tasks_list.curselection()))
        corrected_viewed = task_to_be_viewed[2:]

        conn = sqlite3.connect('tasks.db')
        cursor = conn.cursor()

        cursor.execute("SELECT id FROM tasks WHERE REPLACE(REPLACE(REPLACE(task_or_action, '{', ''), '}', ''), '[', '') = ?", (corrected_viewed,))
        row = cursor.fetchone()
        row_id = row[0]
...
def do_task_tomorrow():
    ...
    try:
        if row is None:
            ...
            row_id = row[0]
            cursor.execute("UPDATE tasks SET date=? WHERE id=?", (tomorrow_date, row_id))
            conn.commit()
...
def delete_task_from_database():
    global result_tasks
    task_to_be_deleted = today_tasks_list.get((today_tasks_list.curselection()))
    ...
    try:
        if row is None:
            ...
            answer = messagebox.askyesno("Delete Task", "Do you want do delete task?")
            if answer:
                row_id = row[0]
                cursor.execute("DELETE FROM tasks WHERE id = ?", (row_id,))
                conn.commit()
...
def progress_bar_of_day():
    try:
        ...
        if total_number_tasks == 0:
            style_progressbar.configure("green.Horizontal.TProgressbar", background= '#970000', troughcolor='#20EE00')             
        elif 1 <= value <= 30:
        ...
        style_progressbar = ttk.Style()
        style_progressbar.theme_use("default")
        style_progressbar.configure("green.Horizontal.TProgressbar", background= '#970000', troughcolor='#970000')
        today_progress_bar = ttk.Progressbar(top_header_frame_left, style = "green.Horizontal.TProgressbar",  orient="horizontal", length=249, mode="determinate")
        today_progress_bar.place(x = 0, y = 80)
        today_progress_bar["value"] = value
